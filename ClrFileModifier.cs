...
{
    public class ClrFileModifier
    {
        public ClrFileModifier(string s, string path)
        {
            m_OriginalFile = s;
            m_OutputPath = path;
        ...
        public static List<string> ResultTexts
        {
            get { return s_ResultTexts; }
        }
        ...
        public void Extend(string fullName, string methodName, uint insertSize)
        {
            ...
            if (!srcValid) {
                s_ResultTexts.Add(string.Format("Can't find src {0}->{1}", className, srcValid));
                return;
            }
        ...
        public void ModifyWithField(string fullName, string methodName, uint pos, byte op, string fullCn, string fn)
        {
            ...
            Modify(classNs, className, methodName, (uint offset) => {
                ...
                if (!srcValid) {
                    s_ResultTexts.Add(string.Format("Can't find src {0}->{1}", cn, srcValid));
                    return;
                }
        ...
        public void Replace(string src, string target)
        {
            ...
            if (!srcValid || !targetValid) {
                s_ResultTexts.Add(string.Format("Can't find src {0}->{1} or target {2}->{3}", srcName, srcValid, targetName, targetValid));
                return;
            }
            MethodDefIterator it = m_ClrFile.Metadata.Tables[(int)CorTokenType.mdtMethodDef] as MethodDefIterator;
            for (uint ix = (uint)srcStart; ix < (uint)srcEnd; ++ix) {
                ...
                if (MoveToMatchedMethod(it, targetStart, targetEnd)) {
                    ...
                    s_ResultTexts.Add(string.Format("Replace {0}->{1} succeeded", srcName, targetName));
                }
            }
        }
        public void ReplaceAssemblyRefName(string src, string target)
        {
            var it = m_ClrFile.Metadata.Tables[(int)CorTokenType.mdtAssemblyRef] as AssemblyRefIterator;
            it.MoveToFirst();
            do {
                string tmp = GetString(m_ClrFile.Buffer, m_StringStart, it.Name);
                if (tmp == src) {
                    if (ReplaceString(m_ClrFile.Buffer, m_StringStart, it.Name, target)) {
                        s_ResultTexts.Add(string.Format("Replace string {0}->{1} succeeded", src, target));
                    }
                    else {
                        s_ResultTexts.Add(string.Format("Replace string {0}->{1} failed", src, target));
                    }
                    break;
                }
            } while (it.MoveToNext());
        }
        public void RedirectAssembly(string target, IList<string> srcs)
        {
            uint targetName = 0;
            var it = m_ClrFile.Metadata.Tables[(int)CorTokenType.mdtAssemblyRef] as AssemblyRefIterator;
            it.MoveToFirst();
            ...
            if (targetName > 0) {
                ...
                do {
                    string tmp = GetString(m_ClrFile.Buffer, m_StringStart, it.Name);
                    if (srcs.Contains(tmp)) {
                        it.Name = targetName;

                        s_ResultTexts.Add(string.Format("Redirect assembly {0}->{1} succeeded", tmp, target));
                    }
                } while (it.MoveToNext());
            }
            else {
                s_ResultTexts.Add(string.Format("Redirect asssembly to {0} failed", target));
            }
        }
        ...
        private static bool ReplaceString(byte[] buffer, uint start, uint offset, string target)
        {
            ...
            if (nlen <= 0 || nlen < bytes.Length)
                return false;
            for (int ix = 0; ix < nlen; ++ix) {
                if (ix < bytes.Length)
                    buffer[pos + ix] = bytes[ix];
                else
                    buffer[pos + ix] = 0;
            }
            return true;
        ...
        private static List<String> s_ResultTexts = new List<string>();
    }
    public static class ScriptProcessor
    {
        ...
        public static void BeginFile(string file, string info)
        {
            ...
            if (!s_ClrFileModifiers.ContainsKey(file)) {
                ClrFileModifier methodBodyModifier = new ClrFileModifier(file, s_OutputPath);
                s_ClrFileModifiers[file] = methodBodyModifier;
            }
        }
        ...
        {
            s_ClrFileModifiers[file].BeginReplace();
        }
        public static void EndReplace(string file)
        {
            s_ClrFileModifiers[file].EndReplace();
        }
        public static void Replace(string file, string src, string target)
        {
            s_ClrFileModifiers[file].Replace(src, target);
        }
        public static void ReplaceAssemblyRefName(string file, string src, string target)
        {
            s_ClrFileModifiers[file].ReplaceAssemblyRefName(src, target);
        }
        public static void RedirectAssembly(string file, string target, IList<string> srcs)
        {
            s_ClrFileModifiers[file].RedirectAssembly(target, srcs);
        }
        public static void BeginExtend(string file, uint newSize)
        {
            s_ClrFileModifiers[file].BeginExtend(newSize);
        }
        public static void Extend(string file, string fullClassName, string methodName, uint insertSize)
        {
            s_ClrFileModifiers[file].Extend(fullClassName, methodName, insertSize);
        }
        public static void EndExtend(string file)
        {
            s_ClrFileModifiers[file].EndExtend();
        }
        public static void BeginModify(string file)
        {
            s_ClrFileModifiers[file].BeginModify();
        }
        public static void EndModify(string file)
        {
            s_ClrFileModifiers[file].EndModify();
        }
        public static void WriteLoadArg(string file, string fullClassName, string methodName, uint pos, int index)
        {
            if (index >= 0 && index <= 3) {
                s_ClrFileModifiers[file].ModifyWithData(fullClassName, methodName, pos, new byte[] { (byte)(0x02 + index) });
            } else if (index <= 0xff) {
                s_ClrFileModifiers[file].ModifyWithData(fullClassName, methodName, pos, new byte[] { 0x0E, (byte)index });
            } else if (index <= 0xffff) {
                s_ClrFileModifiers[file].ModifyWithData(fullClassName, methodName, pos, new byte[] { 0xFE, 0x09, (byte)(index & 0xff), (byte)((index >> 8) & 0xff) });
            }
        }
        public static void WriteLoadLocal(string file, string fullClassName, string methodName, uint pos, int index)
        {
            if (index >= 0 && index <= 3) {
                s_ClrFileModifiers[file].ModifyWithData(fullClassName, methodName, pos, new byte[] { (byte)(0x06 + index) });
            } else if (index <= 0xff) {
                s_ClrFileModifiers[file].ModifyWithData(fullClassName, methodName, pos, new byte[] { 0x11, (byte)index });
            } else if (index <= 0xffff) {
                s_ClrFileModifiers[file].ModifyWithData(fullClassName, methodName, pos, new byte[] { 0xFE, 0x0C, (byte)(index & 0xff), (byte)((index >> 8) & 0xff) });
            }
        }
        public static void WriteLoadField(string file, string fullClassName, string methodName, uint pos, string cn, string fn)
        {
            s_ClrFileModifiers[file].ModifyWithField(fullClassName, methodName, pos, 0x7B, cn, fn);
        }
        public static void WriteLoadStaticField(string file, string fullClassName, string methodName, uint pos, string cn, string fn)
        {
            s_ClrFileModifiers[file].ModifyWithField(fullClassName, methodName, pos, 0x7E, cn, fn);
        }
        public static void WriteCall(string file, string fullClassName, string methodName, uint pos, string cn, string fn)
        {
            s_ClrFileModifiers[file].ModifyWithCallMethod(fullClassName, methodName, pos, 0x28, cn, fn);
        }
        public static void WriteCallVirt(string file, string fullClassName, string methodName, uint pos, string cn, string fn)
        {
            s_ClrFileModifiers[file].ModifyWithCallMethod(fullClassName, methodName, pos, 0x6F, cn, fn);
        }
        public static void WriteNops(string file, string fullClassName, string methodName, uint pos, int size)
        {
            ...
            s_ClrFileModifiers[file].ModifyWithData(fullClassName, methodName, pos, nops);
        }

        private static DslCalculator s_Calculator = new DslCalculator();
        private static Dictionary<string, ClrFileModifier> s_ClrFileModifiers = new Dictionary<string, ClrFileModifier>();
